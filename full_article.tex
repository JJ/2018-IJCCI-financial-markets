...
\keywords{Stock exchange, fuzzy systems, stock trading, genetic
  programming} 

\abstract{% First, motivation: Why do you need this paper
  % Second, problem: what are you dealing with.
  % Start: Motivation - AHA :)
  Making decisions when trading in a financial market is a complex
  task, involving the analysis of different factors. A solution to
  this complex process is provided in this paper: %
  % 
  a decision support system, which helps users
  choose a stock to buy. In order to arrive to a decision, an 
  architecture of zero-sum multi-agents based on fuzzy
  inference systems and genetic programming was designed.  Each of the agents uses a fuzzy inference system to decide in what
  direction they will trade and how much trading force they will
  use. This
  architecture allows a person %individual == agent?    % No, a human
                               %being. The idea here is that you can
                               %check how the membership functions
                               %were evolved and use them to create a
                               %new theory about the market. For
                               %example, if the majority of the agents
                               %were buying when X happened, let's
                               %check how the membership functions'
                               %configuration affected this
                               %decision. - AHA
  %OK
  to examine the evolved membership
  functions of the fuzzy inference systems to better understand the
  behavior of a financial market.
  The multi-agent system % the whole system? - JJ   % Yes, all
                              % the agents start evolving to curve fit
                              % the market prices. For example, if the
                              % price at point t0 is 10, and agent1 is
                              % buying 15 units, then agent2 needs to
                              % sell 5 units to arrive at the price of
                              % 10. - AHA
  ...
  method was tested and was found profitable against a stock selection benchmark based on the
  Dow Jones Index.} % Is that the only metric?  Being profitable? - JJ
                    % % Unfortunately, yes. As far as I remember, I
                    % avoided focusing on error minimization, for
                    % example (regarding the curve fitting), because
                    % it wasn't giving great results compared to other
                    % curve fitting solutions. - AHA
% ok - Jj
...
black box. % Justification to the selection of technologies/models - AHA
...
This dataset was selected because it contains recent traiding data
and it is available in an open repository.
% Justify this choice. - JJ
% This dataset was chosen because it is relatively recent.
% Basic justification added - Mario 
...
% Summary of the structure of the dataset: number of elements, number
% of days, number of stocks... - JJ
...
% This should be explained better. Say something about evolving the
% communities, genetic material and so on. It's a conference on
% computational intelligence. - JJ
...
\ref{flowchart-without-dynamic}. % Maybe explain this GA a bit better?
                                ...
                                % It's not published. I made it from
                                % scratch - AHA
...
a number of five membership functions gave good results in most cases. % 5 membership
                                % functions? - JJ   % Yeah, membership
                                % functions. The idea is that, as the
                                % membership functions are being
                                % evolved, they don't really mean
                                % something like "high" or "low." We
                                % just want to map an antecedent to a
                                % consequent, and then perform the
...
defuzzification. % Reworded the comented paragraph above - AHA
The FIS
...
MF created by the GP algorithm and, similarly, GPcons\text{N}
...
selected from the population. % I'm a bit lost. A clearer, top-down
                              % explanation of the algorithm should be
                              % done. What is the population? What
                              % does GP do here? What is the fitness? 
...
\ref{flowchart-without-dynamic}. %Maybe start with this and explain it
                                %top-down. 
...
% This has to be explained better. I'm lost again. I though we were
% using fixed MFs. We use a GA for the community and GP for the
% membership functions? All individuals have the same one? - JJ

% We use GP to evolve a function that represents the MF (that's why
% they look ugly), and an evolutionary algorithm that migrate the
% communities of agents. If I ever said that this was a genetic
...
\text{x}, which takes the value of the current input (ADX, RSI or
SO). 
...
% dynamic adaptation of the replacement rate, two experiments were
...
% % I don't think this table really helps with anything.
...
7.075\%. % Why only 8 trials? How did you compute that average? By
         % averaging rate of returns? Or actual money obtained? What
         % happened with trial #3? - JJ  

         % Brown used 8 trials, so I
         % used the same amount. This is explained in the second and third
         % paragraphs of this Section. 

         % It is just a simple average, and yeah, I averaged the rate
         % of returns. I don't know how to better explain it. In my
         % opinion it is enough by saying "the average rate of
         % return"
% But it will depend on the amount of money you obtain. It's not the
% same if you get 10% rate on 1000$ and then 0.1% on 10$. The average
% must be over the total money invested and obtained- JJ

% I'm a little lost with this. "if you get 10% rate on $1000 and then
% 0.1% on $10". The algorithm does a stock selection. This means that
% it will tell you what stock to buy at point t in time. You just buy
% and hold that stock, and after certain period of time, you just
% calculate what percentage or rate of return you got. If the market
% went N points up, this will represent a percentage of what you
% invested. - AHA
...
return. %  I just reworded a bit the paragraph above. - AHA
...
method never achieved a negative average rate of return. % Some
                                % explanation? - JJ
                                %  I think that my motivation here was
                                %  to show that the algorith behaves
                                %  like other learning algorithms:
                                %  with a low number of iterations or
                                %  generations you'll get an
                                %  under-trained model, with a
                                %  moderate amount you'll get good
                                %  results, and with very high number
                                %  of iterations it'll get bad. - AHA
%
...
well, and a high number of iterations creates an over-trained model. % Reworded the commented paragraph above - AHA
...
%Experiments were performed for each of the 30 stocks provided in the
%dataset. As in previous experiments, 12 weeks were used as training
%and the next 13 weeks were used for testing. The proposed method tries
%to predict if the trader has to sell or buy each of the stocks, and
%for each successful or unsuccessful prediction, a positive or negative
%rate of return is accumulated, respectively. This process is repeated
%30 times for each stock, and the results are presented in Table
%\ref{avg-rates-of-return-table}. %Nothing about this result - JJ

% This was work in progress with out good results yet 
...
% Regarding the validation of the method, more experiments can
...
% Structure:
% * Repeat the main objectives of the paper
% * Explain the results and how they are better than the state of the
% art
% * Explain what is the most interesting feature of the algorithm:
% MAS, FIS, GP, adaptive replacement rate (there are too many things,
% I guess, anyway).
